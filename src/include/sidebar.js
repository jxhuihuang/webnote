...
const {
  ...
  activity = [],
  archivePrivate,
} = BootCommons; //isInMaintain:显示的页面  false 博客页面  true 维护页面
const { TabPane } = Tabs;
  ...
  class SideBar extends Component {
    ...
    componentWillMount() {
      ...
      ajaxFa.call(
        ...
        {},
        (data) => {
          const $str = $("<code></code>").append(
            $((data || "").replace(/\<script.*?\>/g, ""))
          );
          // 找找看
          if (
            $str.find("#sidebar_search").length > 0 &&
            $str.find("#sidebar_search_box").length > 0
          ) {
            if (!isMobile) {
              let sidebar_search_html =
                $str.find("#sidebar_search_box").html() || "";
              this.setState(
                {
          ...
          /*****获取分类 */
          if (
            $str.find("#sidebar_postcategory").length > 0 ||
            $str.find("#sidebar_articlecategory").length > 0
          ) {
            let categoryArry = [];
            /*****获取随笔分类 */
          ...
          if ($str.find("#sidebar_scorerank").length > 0) {
            ...
            let liScore_String = $scorerank
              .find(".liScore")
              .html()
              .replace(/(^\s*)|(\s*$)/g, "");
            let liScore = liScore_String
              .split("-")[1]
              .replace(/(^\s*)|(\s*$)/g, "");
            let liRank_String = $scorerank
              .find(".liRank")
              .html()
              .replace(/(^\s*)|(\s*$)/g, "");
            let liRank = liRank_String
              .split("-")[1]
              .replace(/(^\s*)|(\s*$)/g, "");
            $this.setState({
          ...
          /*******获取档案 */
          if (
            $str.find("#sidebar_postarchive").length > 0 ||
            $str.find("#sidebar_articlearchive").length > 0
          ) {
            // archives
            let archiveArry = [];
            /****随笔档案****/
            if ($str.find("#sidebar_postarchive").length > 0) {
              ...
              for (let i = 0; i < $postarchive.find("li").length; i++) {
                ...
                let values = $_this
                  ...
                  .replace(/(^\s*)|(\s*$)/g, "");
                let name = remLastbrackval(values);
                let num = getLastbrackval(
                  $_this.html().replace(/<[^>]+>/g, "")
                ); //获取最后括号的数字;
                let link = $_this.find("a").attr("href");
                name = name.replace(/(^\s*)|(\s*$)/g, "");
                if (name !== "更多") {
          ...
          if ($str.find("#sidebar_recentcomments").length > 0) {
            ...
            for (
              let i = 0;
              i < $recentcomments.find(".recent_comment_title").length;
              i++
            ) {
              let $this = $recentcomments.find(
                ".recent_comment_title:eq(" + i + ")"
              );
              ...
              let comment_user = $this.next().next().html()
                ? $this.next().next().html().replace("--", "")
                : "";
              if (i < 5) {
                recentcommentsArry.push({
                  post_title,
                  post_link,
                  comment_body,
                  comment_user,
          ...
          if ($str.find("#sidebar_categories").length > 0) {
            ...
            for (
              let j = 0;
              j < $str.find("#sidebar_categories .sidebar-block").length;
              j++
            ) {
              let $_links = $str.find(
                "#sidebar_categories .sidebar-block:eq(" + j + ")"
              );
        ...
        ctx + "/ajax/sidecolumn.aspx",
        { dataType: "text", types: "GET" }
      );
      /*****************获取 阅读排行、 评论排行 ********************************/
      ajaxFa.call(
        $this,
        ...
        (data) => {
          ...
          if (
            $str.find("#sidebar_topviewedposts").length > 0 &&
            $str
              .find("#sidebar_topviewedposts")
              .html()
          ...
          /** 评论排行榜 **/
          if (
            $str.find("#sidebar_topcommentedposts").length > 0 &&
            $str
              .find("#sidebar_topcommentedposts")
              .html()
              .replace(/(^\s*)|(\s*$)/g, "").length > 0
          ) {
          ...
          /******推荐排行榜 */
          if (
            $str.find("#sidebar_topdiggedposts").length > 0 &&
            $str
              .find("#sidebar_topdiggedposts")
              .html()
              .replace(/(^\s*)|(\s*$)/g, "").length > 0
          ) {
        ...
        <Fragment>
          <div className="blog_aboutUs sidebar-block">
            <div className="news_avatar">
              <figure>
                <a href={"https://home.cnblogs.com/u/" + blogInfo.blogApp}>
                  <img src={blogInfo.blogAvatar} />
                </a>
              </figure>
              <span>
                <p className="names">
            ...
            <div class="news_list">
              ...
              <p>
                ...
                <span>{blogInfo.joinTime}</span>
              </p>
            </div>
            <div class="news_statistic">
              {blogInfo.post_count && (
                <div>
                  <p class="statistic_num">
                    <a href={ctx + "/p"}>{blogInfo.post_count}</a>
                  </p>
            ...
            <div class="btn-user">
              <div
                class="followsates"
                onMouseOver={this.followMouseover}
                onMouseOut={this.followMouseout}
              ...
              <div class="sendMessage">
                <a href={"https://msg.cnblogs.com/send/" + blogInfo.blogApp}>
                  联系他
                </a>
              </div>
            </div>
          ...
          {activity.map((obj, i) => {
            if (obj.isShow && obj.icon) {
              return (
                <div
                  class="blog-activity sidebar-block"
                  key={i}
                >
          ...
          {zzkHtml.length > 0 && (
            <div className="blog-zzks sidebar-block">
              <h3>搜索</h3>
              <div
                className="zzks-content"
                dangerouslySetInnerHTML={{ __html: zzkHtml }}
              ></div>
            </div>
          )}
          {/* 日历 */}
          {calendarHtml.length > 0 && (
            <div
              className="blog-calendars sidebar-block"
            ...
            <div className="blog-scorerank  sidebar-block">
              ...
              <ul>
                {blogInfo.post_count && (
                  <li>
                    <label>随笔数：</label>
                    <span>{blogInfo.post_count}</span>
                  </li>
                )}
                {blogInfo.article_count && (
                  <li>
                    <label>文章数：</label>
                    <span>{blogInfo.article_count}</span>
                  </li>
                )}
                {blogInfo.comment_count && (
                  <li>
                    <label>评论数：</label>
                    <span>{blogInfo.comment_count}</span>
                  </li>
                )}
                {blogInfo.joinData && (
                  <li>
                ...
                {blogInfo.followers && (
                  <li>
                    <label>粉丝数：</label>
                    <span>{blogInfo.followers}</span>
                  </li>
                )}
                {blogInfo.followees && (
                  <li>
                    <label>关注数：</label>
                    <span>{blogInfo.followees}</span>
                  </li>
                )}
                <li>
                  <label>积分：</label>
                  <span>{scorerank.liScore}</span>
                </li>
                <li>
                  <label>排名：</label>
                  <span>{scorerank.liRank}</span>
                </li>
              </ul>
            </div>
          )}
          ...
          {archives.length > 0 && (
            <div className="blog-archives  sidebar-block">
              {archives.length === 1 ? (
                <Fragment>
                  {" "}
                  {/**随笔档案 postarchive,  文章档案  articlearchive*/}
                  <h3>{archives[0].title}</h3>
                  <ul
                    style={{
                      height:
                        archives[0].list.length > 12 ? archivesheight : "auto",
                    }}
                  >
                    ...
                    return (
                      ...
                      >
                        <ul
                          style={{
                            height:
                              objs.list.length > 12 ? archivesheight : "auto",
                          }}
                        >
                          {objs.list.map((obj, i) => {
                            return (
                ...
                {recentcomment.map((obj, i) => {
                  return (
                    <li key={i}>
                      ...
                      <div
                        class="sidecomments_body"
                        dangerouslySetInnerHTML={{ __html: obj.comment_body }}
                      ></div>
                      <div class="sidecomments_post">
                        来自:<a href={obj.post_link}>{obj.post_title}</a>
                      </div>
                    </li>
                  );
                })}
