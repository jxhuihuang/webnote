  ...
  return new Promise(function (resolve, reject) {
    ...
    ajaxFa.call(
      ...
      (data) => {
        ...
        const UserInfoData = $("<code></code>").append(
          $((data || "").replace(/\<script.*?\>/g, ""))
        );
        let getdata = $(UserInfoData).html().replace(/\s/g, "");
        let flows = getdata.substring(
          getdata.indexOf("('") + 2,
          getdata.indexOf("')")
        );
...
function getcbpdes(str) {
  ...
  if ($this.find(".c_b_p_desc").length > 0) {
    ...
    $this.find(".detail-content").length > 0 ||
...
function getlistfootInfo(str, whichpage) {
  ...
  switch (subPages) {
    case "index":
      ...
      break;
    case "postList":
      let postList_dateTimes = $this
        .find("a[title='permalink']")
        .html()
        .replace(/(^\s*)|(\s*$)/g, "");
      ...
      //获取评论数
      const postList_comentsVal =
        $this.find(".post-comment-count").html() || "";
      ...
      articleObject.editLink = isadmin
        ? $this.find("a:contains('编辑')").attr("href")
        : ""; //获取编辑
      break;
    case "articlesList":
      let articlesList_dateTimes = $this
        .find("a[title='permalink']")
        .html()
        .replace(/(^\s*)|(\s*$)/g, "");
    ...
    case "tagPostlist":
    case "diaryList": //日志列表
      var allList_metaInfo = $this.html()
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (isLogin && !isadmin) {
      $.ajax({
        ...
        type: "get",
        success: function (n) {
          const InfoData = $("<code></code>").append(
            $(n.replace(/\<script.*?\>/g, ""))
          );
    ...
    onOk() {
      $.ajax({
        ...
        success: function (n) {
          if (n == "未登录") {
            linkTo(
              "https://passport.cnblogs.com/login.aspx?ReturnUrl=" + currentUrl
            );
            return false;
          }
        ...
        error: function (n) {
          n.status > 0 &&
            showFollowMsg("抱歉！发生了错误！麻烦反馈至contact@cnblogs.com");
        },
      });
    },
    onCancel() {},
...
function getCommentNum(cb_entryId) {
  ...
  return new Promise(function (resolve, reject) {
    ajaxFa.call(
      ...
      ctx + "/ajax/GetCommentCount.aspx?postId=" + cb_entryId,
      { types: "GET" }
    );
  });
}
...
function getPhotoList() {
  let lenths = $("#GalleryThumbNailViewer1_ThumbNails").find(
    ".divPhoto"
  ).length;
  let photoList = [];
  for (let i = 0; i < lenths; i++) {
    let $this = $("#GalleryThumbNailViewer1_ThumbNails").find(
      ".divPhoto:eq(" + i + ")"
    );
...
function getPages() {
  ...
  if ($(".forFlow").find(".topicListFooter").length > 0) {
    $page_main =
      $(".forFlow").find(".topicListFooter .pager").length > 0
        ? $(".forFlow").find(".topicListFooter .pager")
        : $(".forFlow").find(".topicListFooter");
  ...
  if ($page_main.find("#nav_next_page").length > 0) {
    ...
    if (isMobile) {
      let prePage_href =
        $page_main.find("a:contains('上一页')").attr("href") || "";
      let nextPage_href =
