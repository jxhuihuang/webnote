  ...
  componentDidMount() {
    ...
    switch (webpages) {
      case "index":
        ...
        pageMain != ""
          ? $(".forFlow").append(`<div class="page-content">${pageMain}</div>`)
          : "";
        break;
      case "list":
        let pageType = "";
        ...
        if (list_pagemain && list_pagemain != "") {
          $(".forFlow").append(
            `<div class="page-content">${list_pagemain}</div>`
          );
        }
      ...
      case "tag":
        $("#home").show();
        sideBar(blogInfo); //加载侧边栏
        $("#MyTag1_dtTagList").before(
          '<div class="white boxsizing tagMain"></div>'
        );
        ...
        $(".forFlow").delegate(
          "#tags_orderby_usecount, #tags_orderby_name",
          "click",
          function () {
            setTimeout(() => {
              let tagLists = getTagList() || []; // 获取显示所有tag列表
      ...
      case "diaryInfo": //日志详情
        ...
        articleObject.editLink =
          $(".postDesc a").length > 1
            ? $(".postDesc a:eq(1)").attr("href")
            : $(".postDesc a:eq(0)").attr("href"); //获取编辑
        var addToWzs =
          $(".postDesc a").length > 1
            ? $(".postDesc a:last").attr("onclick")
            : "";
      ...
      case "photolist": //相册列表
        ...
        var xctitle = xctitle1.replace(
          blogTitle,
          xc_title + " - 相册 - " + blogTitle
        );
